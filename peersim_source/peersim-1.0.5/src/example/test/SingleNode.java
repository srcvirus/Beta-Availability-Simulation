...
protected boolean grp_flag;
protected boolean explored;
protected boolean grouping_done;
protected boolean group_processing;
protected boolean alive_once;
protected int explore_cycle=0;
protected ArrayList<KnownNode> knownList;
...
* the configuration.
*/
public SingleNode(String prefix) {

        LoadData.load();
        knownList=new ArrayList<KnownNode>();
...
public Object clone() {
	...
	SingleNode result = null;
        ...
        slot=new double[slot_count];
        slot_single=new double[slot_count];
        knownList=new ArrayList<KnownNode>();
	try { result=(SingleNode)super.clone(); }
	...
	for(int i=0; i<protocol.length; ++i) {
        ...
        {
            slot[i]=LoadData.peer_slot[node_count][i];
            slot_single[i]=LoadData.peer_slot[node_count][i];
        }
        if(result.ID==162)
             System.out.println();

        GlobalData.golayCode.AddToPlexus(result);
        node_count++;
	return result;
}
...
public SingleNode copy() {
	...
	for(int i=0; i<protocol.length; ++i) {
        ...
        grp_size=this.grp_size;
        grp_flag=this.grp_flag;
        explored=this.explored;
        alive_once=this.alive_once;
        group_processing=this.group_processing;
        grouping_done=this.grouping_done;
        for(int i=0;i<slot_count;i++)
        {
            slot[i]=this.slot[i];
            slot_single[i]=this.slot_single[i];
        }
        GlobalData.golayCode.AddToPlexus(result);
	return result;
}
...
public boolean getGrpFlag() { return grp_flag; }
//------------------------------------------------------------------
