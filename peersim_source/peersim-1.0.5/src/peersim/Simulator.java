 ...
 * them are the following: <ul> <li>{@link CDSimulator}: if
 ...
 * returns true </li> </ul> This list represents the order in which these
 * alternatives are checked. That is, if more than one return true, then the
 * first will be taken. Note that this class checks only for these clues and
...
// ----------------------------------------------------------------------
     ...
     * not re-initialized between experiments. <p> Loading the configuration is
     * currently done with the help of constructing an instance of
     * {@link ParsedProperties} using the constructor
     ...
     * to initialize the configuration. <p> After loading the configuration, the
     * experiments are run by invoking the appropriate engine, which is
     * identified as follows: <ul> <li>{@link CDSimulator}: if
     * {@link CDSimulator#isConfigurationCycleDriven} returns true</li>
     * <li>{@link EDSimulator}: if
     * {@link EDSimulator#isConfigurationEventDriven} returns true </li> </ul>
     * <p> This list represents the order in which these alternatives are
     * checked. That is, if more than one return true, then the first will be
     * taken. Note that this class checks only for these clues and does not
     * check if the configuration is consistent or valid.
     *
